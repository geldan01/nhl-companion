...
- [ ] **0.1 Allow-list NHL CDN hosts in `next.config.ts`.** Inspect the existing fixtures (`src/lib/nhl/**/__fixtures__/*.json`) for `headshot`, `logo`, and `image` fields and extract the unique hostnames. Add them to `images.remotePatterns` in `next.config.ts` (typically `assets.nhle.com` for logos and `assets.nhle.com/mugs/...` for headshots — verify against fixtures, don't trust this comment). Verify by importing one URL into a temporary `<Image>` in a throwaway page and confirming `next dev` doesn't 400 on it. Delete the throwaway before the commit.
...
- *(none yet — first entries land in Phase 0.)*
