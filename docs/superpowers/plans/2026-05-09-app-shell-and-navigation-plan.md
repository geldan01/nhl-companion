...
- **2026-05-09 — Phase 1.6.** Tailwind v4's IDE extension flags `text-[var(--text-muted)]` and suggests the canonical `text-(--text-muted)` form. Both work; `npm run lint` doesn't flag the bracket form. Sticking with the bracket form for now (consistency with everything written so far); if more warnings stack up, do one repo-wide sweep instead of trickling.
